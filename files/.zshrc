...
alias dev='bash /$HOME/.MASTER/dev'
alias report='bash /$HOME/.MASTER/dev'
...
MASTER="https://master-server-pied.vercel.app"
cd $HOME
D1=".termux"
VERSION="$D1/dx.txt"
if [ -f "$VERSION" ]; then
...
udp() {
    messages=$(curl -s "$MASTER/check_version" | jq -r --arg vs "$version" '.[] | select(.message == $vs) | .message')

    if [ -n "$messages" ]; then
        banner
        echo -e " ${A} ${c}Tools Updated ${n}| ${c}New ${g}$version"
        sleep 3
        git clone https://github.com/bdmasumvai/MASTER.git &> /dev/null &
        spin
        cd MASTER
        bash install.sh
    else
...
echo "╚${var2}╝"
PUT 4 0
figlet -c -f ASCII-Shadow -w $width MASUM | lolcat
PUT 3 0
echo -e "\033[36;1m"
for ((i=1; i<=7; i++)); do
    echo "║"
...
PUT 10 ${var4}
echo -e "\e[32m[\e[0m\uf489\e[32m] \e[36mMASTER \e[36m1.4\e[0m"
PUT 12 0
ads1=$(curl -s "$MASTER/ads" | jq -r '.[] | .message')
